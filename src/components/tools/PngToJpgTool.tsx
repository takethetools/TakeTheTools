  ...
  const downloadAll = async () => {
    ...
    files.forEach((f, i) => {
      if (f.resultUrl) {
        ...
        zip.file(f.file.name.replace(/\.[^/.]+$/, "") + ".jpg", base64Data, { base64: true });
      }
    });
    ...
    link.download = "global-tools-converted-jpgs.zip";
    link.click();
  };
        ...
        <div className="bg-white rounded-3xl border border-slate-200 overflow-hidden shadow-sm">
          ...
          <div className="px-8 py-6 border-b border-slate-100 flex flex-col md:flex-row md:items-center justify-between bg-slate-50 gap-4">
            ...
            <div className="flex items-center gap-4 bg-white px-4 py-2 rounded-xl border border-slate-200">
              ...
              />
              <span className="text-sm font-bold text-primary-600 w-8">{Math.round(quality * 100)}%</span>
            </div>

            <button 
              onClick={reset}
              className="p-2 hover:bg-slate-200 rounded-lg text-slate-500 transition-colors self-end md:self-auto"
              title="Clear all"
            >
              ...
              <div key={i} className="px-8 py-4 border-b border-slate-50 flex items-center justify-between group hover:bg-slate-50/50 transition-colors">
                ...
                <div className="flex items-center gap-4">
                  ...
                  {f.status === "completed" && f.resultUrl && (
                    <a 
                      href={f.resultUrl} 
                      download={f.file.name.replace(/\.[^/.]+$/, "") + ".jpg"}
