...
const exampleConfig: DevConfig = {
  runtime: "node",
  port: "8080",
  domain: "takethetools.com",
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setConfig({
                    ...config,
                    runtime: e.target.value as "node" | "python",
                  })
                }
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between px-2">
            <label className="text-xs font-bold text-slate-400 uppercase tracking-widest">
              Generated {mode === "docker" ? "Dockerfile" : "nginx.conf"}
            </label>
            <div className="flex gap-4">
              <button
                onClick={downloadResult}
                className="text-slate-500 hover:text-primary-600 text-xs font-bold flex items-center gap-1 uppercase tracking-wider"
              >
                <Download className="w-3 h-3" /> Download
              </button>
              <button
                onClick={copyResult}
                className="text-primary-600 text-xs font-bold flex items-center gap-1 uppercase tracking-wider"
              >
                {isCopied ? (
